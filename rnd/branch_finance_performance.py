...
branch_finance_performance.show(truncate=False)

# 6. (Opsional) Simpan hasil analisis batch ke Delta atau Parquet
branch_finance_performance.write.format("delta").mode("overwrite").save("./data/analytics/branch_finance_performance/")
# 7. Sink Postgres
pg_writer = pg_writer()
pg_writer.write_to_postgres(branch_finance_performance, table_name="branch_finance_performance")
